...
int main(int argc, char *argv[])
{
	...
	vector<tuple<string, string, vector<indd>, indd>> DataSets; //Name, Filepath, inputcolumns, outputcolumn
	vector<indd> attributeindices;
	...
	const indd maxRules = numPatterns; // maximum explanation table size
	vector<indd> tablesizes = {maxRules}; // explanation table sizes at which to evaluate time and gain
	...
	for (indd repetitions = 0; repetitions < 1; ++repetitions) 
	{ 
		// enable experiment repetition
		for (indd i = 0; i < DataSets.size(); ++i)
		{ 
			// itrerate datasets and parse information
			string dataName = get<0>(DataSets[i]);
			//cout << "=======" << dataName << "========\n";
			string file = get<1>(DataSets[i]);
			vector<indd> dataColumns = get<2>(DataSets[i]);
			indd outcome = get<3>(DataSets[i]);
            //cout << "=======" << outcome << "========\n";

			// Obtain Reference KL-Divergence

			Lighthouse ref(file, dataColumns, outcome, maxRules);
			numm baseKL = ref.KLDivergenceNew();
			

			//== ClassicFL == Parameters: Samplesize//
			if (TOGGLE_FL)
			{
				vector<indd> configsFL = {16}; // Enable easy way to test multiple settings for sample size
				for (indd s : configsFL)
				{
					for (numm minSupport : minSupportParams) {
						
						Lighthouse A(file, dataColumns, outcome, maxRules);
						// cout << "== FL Classic -- " << s << " -- " << minSupport << " ==\n";
						auto start = chrono::high_resolution_clock::now();
						for (indd r = 1; r <= maxRules; ++r)
						{
			...
			if (TOGGLE_LENS)
			{
				vector<pair<indd, indd>> configsLENS; // Easy way to test multiple parameter settings for LENS
				...
				for (auto config : configsLENS)
				{
					for (numm minSupport : minSupportParams) {
						...
						indd s = config.first;
						indd k = config.second;
						Lighthouse A(file, dataColumns, outcome, maxRules);
						// cout << "== Bottom Up Cube -- " << s << ";" << k << " ==\n";
						auto start = chrono::high_resolution_clock::now();
						for (indd r = 1; r <= maxRules; ++r)
						{
							A.LENS(1, internalPrint, s, k, 0, minSupport);
							if (externalPrint && (find(tablesizes.begin(), tablesizes.end(), r) != tablesizes.end()))
							{
								auto now = chrono::high_resolution_clock::now();
								numm KL = A.KLDivergenceNew();
